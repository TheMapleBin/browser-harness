...
- 为了覆盖开机自动连接但 WLAN 事件早于用户登录的情况，登录后 runner 会最多等待 180 秒检测 `ZSClib` 是否已经成为当前 SSID。
- 如果当前已经连接到其他 WLAN，runner 会立即退出，不会继续等待或启动 Chrome。
- 如果 Windows 已报告 `ZSClib` 具备 Internet 连接，runner 会直接退出，不会启动 Chrome。
- PowerShell 以隐藏窗口运行。
...
Runner waits for ZSClib SSID: up to 180 seconds
```
